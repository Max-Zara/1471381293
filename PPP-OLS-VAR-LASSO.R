...
###Now Do LASSO 
#Using the retail.ppp.usdx.fit as base
library(glmnet)
train.x <- retail.ppp[,retail.ppp.usdx.cols]; 
#identify which rows have NAs
train.y <- retail.ppp[complete.cases(train.x),"Retail.Log.Lag.0"]
train.x <- train.x[complete.cases(train.x),]
head(train.x)

cv.lin <- cv.glmnet(as.matrix(train.x), as.matrix(train.y), nfolds = 10)
lin.prediction <- predict(cv.lin, as.matrix(train.x), s= "lambda.min")
MAE(lin.prediction,train.y)/mean(train.y)
plot(lin.prediction,train.y)
#Get coefficients
coef(cv.lin, s="lambda.min")[which(coef(cv.lin,s="lambda.min")!=0),]
...
#See how LASSO Selects Coefficients
lasso.coef.list <- data.frame()
library(plyr)
library(glmnet)
for(i.lag in 12:0){
  cols.to.use <- paste0(c(rep("PPP.Log.Lag.",i.lag+1),rep("Us.Log.Lag.",i.lag+1)),rep(seq(0,i.lag,1),2))
  cols.to.use <- c(cols.to.use,"Dummy.Jan","Dummy.Feb","Dummy.Mar","Dummy.Apr","Dummy.May","Dummy.Jun","Dummy.Jul","Dummy.Aug",
                   "Dummy.Sep","Dummy.Oct","Dummy.Nov")
  retail.ppp.temp <- retail.ppp[,cols.to.use]
  
  train.x <- retail.ppp.temp
  #identify which rows have NAs
  train.y <- retail.ppp[complete.cases(train.x),"Retail.Log.Lag.0"]
  train.x <- train.x[complete.cases(train.x),]
  head(train.x)
  ...
  cv.lin <- cv.glmnet(as.matrix(train.x), as.matrix(train.y), nfolds = 10)
  #Get coefficients
  lasso.coefs <- coef(cv.lin, s= "lambda.min"); t(lasso.coefs)
  lasso.coef.list <- rbind.fill(lasso.coef.list,data.frame(as.matrix(t(lasso.coefs)),Lags = i.lag))
  #coef(cv.lin, s="lambda.min")[which(coef(cv.lin,s="lambda.min")!=0),]
}
lasso.coef.list[lasso.coef.list==0] <- NA
write.csv(lasso.coef.list,file="LassoCoefListNoSeasonal.csv")
write.csv(lasso.coef.list,file="LassoCoefListSeasonal.csv")
